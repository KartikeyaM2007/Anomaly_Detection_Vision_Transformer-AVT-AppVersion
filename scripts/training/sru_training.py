...
try:
    from sru import SRU
    SRU_IMPORT_ERROR = None
except Exception as exc:
    SRU = None
    SRU_IMPORT_ERROR = exc
    ...
    def __init__(self, input_size: int, hidden_size: int, **kwargs):
        ...
        if SRU is None:
            raise RuntimeError(
                ...
                f"Original import error: {SRU_IMPORT_ERROR}"
            )
...
class VADTrainer:
    ...
    def prepare_data(self,
        ...
        # Handle both 2D (num_videos, features) and 3D (num_videos, frames, features)
        if len(file_embeddings.shape) == 2:
            print(f"[INFO] 2D embeddings detected {file_embeddings.shape}. Expanding to 3D...")
            file_embeddings = np.expand_dims(file_embeddings, axis=1)

        if len(file_embeddings.shape) != 3:
            raise ValueError(f"Expected 3D array [videos, frames, features], got {file_embeddings.shape}")
        ...
        # pin_memory=True for faster GPU transfer on RTX 4060
    ...
    def train(self,
        ...
        for epoch in range(epochs):
            ...
            for i, (videos, labels) in progress_bar:
                # (batch, seq, features) -> (seq, batch, features) for SRU
            ...
            if save_best_model and accuracy > best_accuracy:
                ...
                self.save_model('best_model.pth', epoch + 1, accuracy)
    ...
    def evaluate(self, test_loader: DataLoader, show_plots: bool = False) -> Dict[str, Any]:
        ...
        # ROC / PR for binary or multiclass
    ...
    def plot_evaluation_results(self, results: Dict[str, Any]):
        ...
        plt.tight_layout()
        plot_path = os.path.join(self.save_dir, f'evaluation_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
    ...
    def plot_training_history(self):
        ...
        plt.tight_layout()
        plot_path = os.path.join(self.save_dir, f'history_{datetime.now().strftime("%Y%m%d_%H%M%S")}.png')
        plt.savefig(plot_path, dpi=150)
    ...
    def load_model(self, checkpoint_path: str) -> Dict[str, Any]:
        ...
        print(f"Loaded model from epoch {checkpoint['epoch']} | acc: {checkpoint['accuracy']:.2f}%")
        return checkpoint
...
def main():
    ...
    trainer.save_model(
        f'final_sru_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pth',
